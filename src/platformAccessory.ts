...
export class SaveConnectPlatformAccessory {
  ...
  constructor(
    ...
    // Use the existing Switch service to change the ventilation user mode to "Refresh" for
    // some minutes.

    this.refreshService =
      this.accessory.getService('Refresh') ||
      this.accessory.addService(this.platform.Service.Switch, 'Refresh');

    // set the service name, this is what is displayed as the default name on the Home app
    // in this example we are using the name we stored in the `accessory.context` in the `discoverDevices` method.
    const device: SaveConnectDevice = accessory.context.device;
